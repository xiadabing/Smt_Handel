...
class HandleExecl(object):
    ...
    def write_result(self, date, shop, number, country):
...
#     HandleExecl().write_result(date='2021-07-10',shop=list(item.keys())[0],number=list(item.values())[0],country='乌克兰')
...
# if __name__ == '__main__':
    ...
    new_data = data_hanle(a)

    for item in new_data:
        value = list(item.values())[0].split(' ')
        HandleExecl().write_result(date="2021-07-12", shop=list(item.keys())[0], number=value[0], amount=value[1],
                               country='乌克兰')